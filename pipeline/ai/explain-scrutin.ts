/**
 * Scrutin explanation + summary (brief §7). Runs only on the significant
 * scrutins (solennel / motion de censure / vote on the whole text) with the
 * more capable model.
 *
 * Strict grounding: the model relies ONLY on the provided sources (the
 * scrutin's title and subject, and its dossier's title/type). It must say when
 * information is missing rather than invent it, stay strictly neutral, and
 * write in French.
 */
...
Tu t'appuies UNIQUEMENT sur les sources fournies (intitulé et objet du scrutin, titre et type du dossier législatif).
...
- Reste sobre et précis. Pas de formules d'introduction.`;
...
export async function explainScrutin(
  s: ScrutinInput,
  ...
  const text = res.text;
  if (!text) return { explanation: "", summary: "" };
  const parsed = JSON.parse(text) as ScrutinExplanation;
  return {
